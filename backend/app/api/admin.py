...
async def get_database_status(session: Session = Depends(get_session)):
    ...
    try:
        movie_count = session.exec(text("SELECT COUNT(*) as count FROM movies")).first()
        swipe_count = session.exec(text("SELECT COUNT(*) as count FROM swipes")).first()
        user_count = session.exec(text("SELECT COUNT(*) as count FROM users")).first()
        return {
